		...
		.then(async function (blob) {
			...
			await fetch(new Request(dbUrl + "/categories?id=" + product.categoryId))
				...
				.then(async function (blob) {
					...
					pathElement.innerHTML = `<a class="path-link" data-route="">Головна сторінка</a>
            ...
            <a class="path-link" data-route="#categories/${product.subCategoryUrl}">${product.subCategoryName}</a>`;
					...
					productSection.innerHTML = `<div class="vertical-photos-container">
                ${product.images
					.map((image) => `<a class="product-photo" style="background-image: url(product-photos/${image});"></a>`)
					.join("")}
            </div>
            <img src="product-photos/${product.images[0]}" alt="${product.name}" class="full-product-photo">
            <div class="product-description-container">
                ${product.sale ? `<p class="sale-label">Знижка ${product.sale}%</p>` : ``}
                <h1 class="product-title">${product.name}</h1>
                ${
					product.sale
						? `<p class="product-price crossed-out">${product.price.toFixed(2)} грн</p>
                                    <p class="product-price discounted">${((product.price * (100 - product.sale)) / 100).toFixed(
										2
									)} грн</p>`
						: `<p class="product-price">${product.price.toFixed(2)} грн</p>`
				}
                ${product.inStock ? `<div class="in-stock">У наявності</div>` : `<div class="not-in-stock">Немає в наявності</div>`}
                <div class="product-quantity-container">
                    <a class="quantity plus"></a>
                    <input class="quantity-input" value="1">
                    <a class="quantity minus"></a>
                </div>
                <button ${product.inStock ? `` : `disabled`} class="add-to-cart-button" data-addtocart="${
						product.url
					}" data-quantity="1">У кошик</button>
                ...
                <p class="product-description">
					...
					for (let i = 0; i < productPhotos.length; i++) {
						productPhotos[i].addEventListener("click", function () {
							for (let j = 0; j < productPhotos.length; j++) productPhotos[j].classList.remove("active");
							productPhotos[i].classList.toggle("active");
							productSection.querySelector(".full-product-photo").src = `product-photos/${product.images[i]}`;
						});
					}
					...
					productSection.querySelector(".quantity.plus").addEventListener("click", function () {
						if (quantityInput.value) quantityInput.value = (parseInt(quantityInput.value, 10) + 1).toString();
