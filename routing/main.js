	...
	return function (element, config) {
		...
		let setActive = function () {
			...
			states.forEach(function (item, index, arr) {
				...
				if (width >= states[index].minWidth) index = index;
			});
		...
		let transformItem = function (direction) {
			let nextItem,
				currentIndicator = indexIndicator;
...
let constructor = async function (container) {
	...
	pageWidth.innerHTML = `<div class="slider">
	...
	await fetch(new Request(dbUrl + "/actions"))
		...
		.then(async function (blob) {
			...
			for (let i = 0; i < actions.length; i++) {
				...
				actionItems[
					i
				].innerHTML = `<div href="javascript:;" style="background: center url(${actions[i].photo}); background-size: cover;" data-route="#actions/${actions[i].url}"></div>`;
	...
	await fetch(new Request(dbUrl + "/products?recommended=true"))
		...
		.then(async function (blob) {
			...
			for (let i = 0; i < 8; i++) {
				...
				if (i < 6) {
					goods[i].classList.add("hit-goods-grid-container", "one-third");
				} else {
					goods[i].classList.add("hit-goods-grid-container", "one-half");
				}
				goods[
					i
				].innerHTML = `<a href="javascript:;" class="hit-goods-grid-item" data-route="#products/${recommendedProducts[i].url}">
