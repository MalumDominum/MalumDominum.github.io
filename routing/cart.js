...
let constructor = async function (container) {
    ...
    <div class="cart">
        ...
        <div class="cart-items">
            <div class="cart-items-header">
                ...
                <div></div>
                <div class="cart-items-header-price">Ціна та кількість</div>
                <div class="cart-items-header-total">Сума</div>
            </div>
        </div>
        <div class="cart-buttons">
            <button id="clearCart" class="okay-button red">Очистити кошик</button>
            <button id="order" class="okay-button">Замовити</button>
        </div>
    </div>
    `;
	...
	if (cartProductsRequest.length) {
		await fetch(new Request(dbUrl + "/products?" + cartProductsRequest))
			...
			.then(async function (blob) {
				...
				dbCartData.forEach(function (currentCartProduct) {
                    ...
                    ${
						currentCartProduct.sale
							? `<div class="cart-items-single-price">${price} <span>(${displayPrice})</span> грн * ${quantity}</div>`
							: `<div class="cart-items-single-price">${price} грн * ${quantity}</div>`
					}
                    ${
						currentCartProduct.sale
							? ` <div class="cart-items-single-total">${price * quantity} <span>(${
                ...
                <div class="cart-items-header-product">Всього:</div>
                <div></div>
		...
		pageWidth.querySelector(".cart > h2").innerHTML = "Ваш кошик порожній";
